...
suite("Extension Test Suite", () => {
	...
	test("The extension is installed", () => {
		...
		const ourId = "genexus.kasstor-lit-vscode-plugin";
	...
	test("We produce a diagnostic", async () => {
		...
		await config.update("kasstor-lit-vscode-plugin.logging", "verbose", true);
		await config.update("kasstor-lit-vscode-plugin.rules.no-missing-element-type-definition", "error", true);
	...
	test("We detect no-missing-import properly", async () => {
		const config = vscode.workspace.getConfiguration();
		await config.update("kasstor-lit-vscode-plugin.logging", "verbose", true);
		await config.update("kasstor-lit-vscode-plugin.rules.no-missing-import", "error", true);
