...
async function main() {
	...
	await writeFile("./built/node_modules/@genexus/kasstor-ts-lit-plugin/package.json", JSON.stringify(tsPluginPackageJson, null, 2));

	// We need to re-export the ts-lit plugin build output from a root index file
	await writeFile("./built/node_modules/@genexus/kasstor-ts-lit-plugin/index.js", `module.exports = require("./lib/index").init;`);

	const pluginPackageJson = require("./package.json");
	// vsce is _very_ picky about the directories in node_modules matching the
	// extension's package.json, so we need an entry for @genexus/kasstor-ts-lit-plugin or it
