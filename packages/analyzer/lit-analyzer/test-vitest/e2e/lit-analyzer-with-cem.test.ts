...
describe("LitAnalyzerContext + CEM + Kasstor library-summary", () => {
	...
	beforeEach(() => {
		...
		cpSync(FAKE_PKG, join(projectRoot, "node_modules", "fake-pkg-with-manifest"), { recursive: true });

		// Kasstor library-summary at <root>/src/library-summary.ts
		mkdirSync(join(projectRoot, "src"), { recursive: true });
		cpSync(join(KASSTOR_FIXTURE, "src", "library-summary.ts"), join(projectRoot, "src", "library-summary.ts"));
	});
	...
	it("loads CEM-described tags from node_modules into htmlStore", () => {
		...
		expect(context.htmlStore.getHtmlTag("fake-button")?.attributes.map(a => a.name)).toContain("label");
	});
