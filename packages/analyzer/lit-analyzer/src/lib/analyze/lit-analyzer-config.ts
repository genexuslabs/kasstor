...
/**
 * Controls how the legacy WCA analyzer is used relative to manifest-based
 * sources (CEM in `node_modules`, Kasstor library summary).
 *
 * - `"auto"` (default): WCA runs only on files NOT covered by a loaded manifest
 *   AND not flagged as external library. Manifests are the primary source.
 * - `"always"`: WCA runs on every uncovered file (legacy behavior — also
 *   re-analyzes external libs even when they ship a CEM).
...
export interface LitExternalManifestsConfig {
	...
	paths?: string[];
	/** Whether to scan node_modules for `package.json#customElements`. Default `true`. */
	scanNodeModules?: boolean;
}

export type LitKasstorSummaryConfig =
	| "auto"
	| false
	| {
			/** Path to the directory whose `library-summary.ts` (or generated equivalent) describes Kasstor components. */
			srcPath: string;
	  };
...
export interface LitAnalyzerConfig {
	...
	/** Kasstor extension: library-summary integration. */
	kasstorSummary: LitKasstorSummaryConfig;
}
...
export function makeConfig(userOptions: Partial<LitAnalyzerConfig> = {}): LitAnalyzerConfig {
	...
	return {
		...
		kasstorSummary: userOptions.kasstorSummary ?? "auto"
	};
}
