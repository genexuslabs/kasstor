...
export class CemNodeModulesSource implements ExternalManifestSource {
	...
	load(ctx: ExternalManifestSourceContext): readonly ResolvedManifest[] {
		...
		if (!projectPkg) {
			ctx.logger.warn(`[cem-node-modules] no package.json at ${projectPkgJsonPath}; skipping scan.`);
			return [];
		}
		...
		for (const depName of directDeps) {
			...
			if (!existsSync(manifestPath)) {
				ctx.logger.warn(`[cem-node-modules] ${depName} declares customElements="${pkgJson.customElements}" but file is missing.`);
				continue;
			}
			...
			if (isLegacyCemSchema(manifest.schemaVersion)) {
				ctx.logger.warn(`[cem-node-modules] ${depName} ships CEM schemaVersion="${manifest.schemaVersion}"; best-effort parse.`);
			}
