...
- **@genexus/kasstor-core: avoid barrel imports.** In code examples, do **not** import from the package root (e.g. `from "@genexus/kasstor-core"`). Use the specific subpaths so the dev server and production build stay fast and side effects are avoided. Prefer:
  - `@genexus/kasstor-core/decorators/component.js` for `Component`, `KasstorElement`
  - `@genexus/kasstor-core/decorators/event.js` for `Event`, `EventEmitter`
  - `@genexus/kasstor-core/decorators/observe.js` for `Observe`
  - `@genexus/kasstor-core/directives/lazy-load.js` for `lazyLoad`
  - `@genexus/kasstor-core/directives/render-by-platform.js` for `renderByPlatform`

- **Kasstor components: JSDoc for autogenerated docs.** When writing or editing a component that uses `@Component` (Kasstor custom element), document it so **@genexus/kasstor-build** (and the Vite plugin) can generate readmes and type declarations correctly:
  - **Class:** Add a JSDoc block above the class with a **brief description** of the component and the **`@access`** tag. Use `@access public` for components that are part of the library’s public API, or `@access private` / `@access protected` / `@access package` when the component is for internal use only. This scope is used when generating per-component docs and when deciding which components are exported in declaration files.
  - **Public API:** Document every **public property** (`@property`), **event** (`@Event()`), and **public method** with JSDoc (summary and, for methods, `@param` / `@returns` as needed). The build extracts these comments to generate the component readme and API tables; missing JSDoc on public members results in incomplete or empty autogenerated documentation.

- **Lit templates: custom event listener names match the property name.** For components that use `@Event()`, the event name dispatched (and used in addEventListener / Lit template listeners) is the **property name as-is** (camelCase). It is **not** converted to kebab-case. In Lit templates, use the exact property name: e.g. `@countChanged=${...}` to listen to the event from `@Event() protected countChanged`. Do **not** write `@count-changed` or any hyphenated form; that would listen for a different event name and would not fire.
