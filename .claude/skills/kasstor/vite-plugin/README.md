...
- **Exported types for the library** — A framework-agnostic core file (e.g. `components.ts`) that re-exports all types used by the components (props, event details, etc.), plus opt-in per-framework JSX type files that augment each framework's `IntrinsicElements` so the components type-check in JSX templates (component properties, `className`/`class`, `style`, `ref`, `aria-*`, `role`, native and custom events). React is generated by default (`components.react.ts`); SolidJS and StencilJS are opt-in. Configure via `fileGeneration.exportTypesForReact` / `exportTypesForSolidJs` / `exportTypesForStencil`.
