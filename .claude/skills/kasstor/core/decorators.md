# Decorators — @genexus/kasstor-core
...
Light DOM (`shadow: false`) — sheets are adopted into the host's nearest `Document`/`ShadowRoot` via [`addGlobalStyleSheet`](../webkit/stylesheets.md#addglobalstylesheet) (reference-counted) and released on `disconnectedCallback`:
