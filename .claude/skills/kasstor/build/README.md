...
- **Exported types for the library** — Generates a framework-agnostic core file (e.g. `components.ts`) that re-exports all types used by the components and declares the `ComponentProperties` / `ComponentPropertiesSolidJS` / `ComponentEvents` namespaces. Gives library consumers one entry point for types.

- **Per-framework JSX types** — Opt-in files that augment each framework's `IntrinsicElements` so the components type-check in JSX templates (component properties, `className`/`class`, `style`, `ref`, `aria-*`, `role`, native event handlers, and custom events). React is generated by default (`components.react.ts`); SolidJS and StencilJS are opt-in. Event handler naming is framework-correct: React uses `on` + the verbatim event name for custom events (native events come from React's `HTMLAttributes`), StencilJS uses `on` + the capitalized event name, and SolidJS uses the namespaced `on:` directive.
...
export type KasstorBuildOptions = {
  ...
  fileGeneration?:
    | false
    | {
        // Core, framework-agnostic types file. Default "components.ts".
        exportTypesForTheLibrary?: string | false;
        // React JSX types. Default "components.react.ts" (on). Imports `react`
        // and re-uses `exportTypesForTheLibrary` (which must also be enabled).
        exportTypesForReact?: string | false;
        // SolidJS JSX types (imports `solid-js`). Opt-in: default `false`.
